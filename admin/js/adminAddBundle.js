...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initPhotoManager() {
        ...
        // Note: no separate upload step here — selectedFile is appended
        // to the same FormData as the rest of the bundle in handleSave().
    }
    ...
    async function handleSave() {
        ...
        try {
            // FormData (not JSON) because the photo needs to ride in the
            // same request as the rest of the bundle fields.
            const formData = new FormData(form);
            formData.set('products', JSON.stringify(bundleProducts));

            if (selectedFile) {
                formData.append('photo', selectedFile);
            }

            const res = await fetch('/api/admin/create_bundle', {
                method: 'POST',
                body: formData
            });
